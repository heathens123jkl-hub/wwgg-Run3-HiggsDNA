...
| `2024_Summer24` | Summer24 | Prompt24 |

NOTE for 2024 data-taking:  the first release of corrections is stored under the directory `2024_Summer24`. It sports:
- Scale and Smearing corrections for electrons and photons (`electronSS_EtDependent_v1.json.gz` and `photonSS_EtDependent_v1.json.gz`)
- SFs for reco electrons (`electron_v1.json.gz`) in the 20-200 GeV ranges
- CSEV and pixel veto for photons (`photon_veto_v1.json.gz`)
- Electron and photon IDs (`electronID_v1.json.gz` and `photonID_v1.json.gz`).The SFs for the EGM IDs are provided in the 20-200 GeV range. 

DIFFERRENCES in evaluate call for SS Run3 corrections:
- 2022-2023 corrections are retrieved as:
`scales = scale_corr.evaluate('scale', run, ScEta, r9, AbsScEta, pt, gain)
 smears = smear_corr.evaluate('smear', pt, r9, AbsScEta)`
- 2024 corrections are retrieved as:
`scales = scale_corr.evaluate('scale', run, ScEta, r9, pt, gain)
 smears = smear_corr.evaluate('smear', pt, r9, ScEta)`
