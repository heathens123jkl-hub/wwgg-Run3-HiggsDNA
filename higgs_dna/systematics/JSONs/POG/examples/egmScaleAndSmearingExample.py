...
parser = argparse.ArgumentParser(description="Example script to demonstrate electron scale and smearing corrections.")
parser.add_argument('--year', type=int, default=2024, help="Year of the data taking (2022, 2023, or 2024)")
...
suitable_runs = {2022: 324219, 2023: 367127, 2024: 380001}
data_run = np.full(N, suitable_runs[year])  # example run number for data
...
###########################################
path_ETDependent = f"../POG/EGM/{year}_Summer{year-2000}/electronSS_EtDependent{'_v1' if year == 2024 else ''}.json"

if not os.path.exists(path_ETDependent):
    os.system(f"gunzip -k {path_ETDependent}.gz")

cset = correctionlib.CorrectionSet.from_file(path_ETDependent)

suffix = {2022: '2022preEE', 2023: '2023preBPIX', 2024: '2024'}

scale_evaluator = cset.compound[f"EGMScale_Compound_Ele_{suffix[year]}"]
smear_and_syst_evaluator = cset[f"EGMSmearAndSyst_ElePTsplit_{suffix[year]}"]
...
# NOTE: different per year usage
if year == 2022 or year == 2023:
    print(f"Scale evaluator inputs: {Fore.RED}'scale', Run number, ScEta, r9, abs(ScEta), pt, seedGain{Style.RESET_ALL}")
    ...
    print(f"Scale evaluator inputs: {Fore.RED}'scale', Run number, ScEta, r9, pt, seedGain{Style.RESET_ALL}")
...
# NOTE: different per year usage
print(f"Smearing evaluator inputs: {Fore.GREEN}'smear', pt, r9, abs(ScEta){Style.RESET_ALL}")
